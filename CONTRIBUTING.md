...
persistence 層でも原則はドメイン型を優先する。判断順序は以下:

1. **そのまま使えるなら使う**: ドメイン型が既に Diesel の `FromSql`/`ToSql` を実装している、あるいは JSONB カラムのように Serde 経由で透過的に扱える場合は、迷わずドメイン型を使う。
2. **実装して使う**: 未実装でも、`FromSql`/`ToSql` の実装コストが小さければ実装してでもドメイン型を使う。ドメイン型を使うメリット（型安全性、意図の明示）はカラム1つぶんの trait 実装を正当化することが多い。
3. **プリミティブ型で妥協**: 対象カラムの型が複雑で trait 実装が著しく煩雑になる場合に限り、Diesel モデル構造体ではプリミティブ型（`String`, `BigDecimal` 等）を使い、persistence を呼び出す側でドメイン型へ変換する。これは「原則からの妥協」であり、安易に選ばない。

JSONB カラムはドメイン型が全て Serde（`Serialize`/`Deserialize`）を実装しているため、`serde_json::to_value()` / `from_value()` 経由で常にドメイン型のまま扱える（上記 1 に該当）。

さらに、認証/認可など「DB 行を読み取った時点で正規化済みのドメイン型でなければ後続処理が安全に書けない」モジュールでは、persistence 層の公開関数がドメイン型に変換した値を返してよい。これは defense-in-depth として境界で異常値を弾く目的である。
