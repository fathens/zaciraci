...
use crate::{ApiResponse, pools::{VolatilityTokensRequest, VolatilityTokensResponse}, stats::{GetValuesRequest, GetValuesResponse, ValueAtTime}, types::TokenAccount};
...
impl ApiClient for BackendClient {
    ...
    async fn health_check(&self) -> Result<(), ApiError> {
        ...
        if response.status().is_success() {
            ...
            Err(ApiError::Server(format!("Health check failed: {}", response.status())))
        }
    }
        ...
        R: serde::de::DeserializeOwned + Send + std::fmt::Debug + Clone,
    {
        ...
        if !response.status().is_success() {
            ...
            return Err(ApiError::Server(format!("HTTP Error {}: {}", status, error_text)));
        }
...
impl BackendClient {
    pub async fn get_volatility_tokens(
        ...
        let response: ApiResponse<VolatilityTokensResponse, String> = self
            .request(reqwest::Method::POST, "/pools/get_volatility_tokens", Some(request))
            .await
