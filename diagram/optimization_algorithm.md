...
2. **上限閾値との距離による判断**:
   ...
   - 距離が「小」より大きい場合: 他の条件を継続チェック

3. **連続評価値による判断**:
   - 連続する評価値が同じ場合（フラット領域）を検出
   - 上限閾値までの距離が「中」以下の場合: 探索終了
   - それ以外の場合: 探索継続

### 距離の定義
- 「小」: 例えば 3〜5 程度の整数値（上限閾値のすぐ手前）
- 「中」: 例えば 10〜15 程度の整数値（比較的上限閾値に近い）

### 重要なポイント
- 常に最良値と最良点を記録し続ける
- 局所的な山を検出しても、上限閾値に十分近くない限り探索は継続
- フラット領域の検出も探索終了の判断材料として活用
