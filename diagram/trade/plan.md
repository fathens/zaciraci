...
#### ✅ Phase 1 & 2a: 基本実装完了（2025-10-10）

**実装完了内容**:
- ✅ `EndpointPool` の実装（`backend/src/jsonrpc/endpoint_pool.rs`、175行）
- ✅ Weighted random selection によるエンドポイント選択
- ✅ Rate limit 検知時の `mark_failed()` 呼び出し（`rpc.rs:67`）
- ✅ 失敗エンドポイントの一時的無効化（5分間、自動リセット）
- ✅ 設定ファイル（TOML）からのエンドポイント読み込み
- ✅ 詳細なログ出力（エンドポイント選択、失敗マーキング）
- ✅ 統合テスト6件（`backend/src/jsonrpc/endpoint_pool/tests.rs`、179行）
  - `test_endpoint_pool_creation`
  - `test_weighted_random_select`
  - `test_mark_failed`
  - `test_available_endpoints_excludes_failed`
  - `test_rate_limit_triggers_endpoint_switch`
  - `test_all_endpoints_failed_resets`
