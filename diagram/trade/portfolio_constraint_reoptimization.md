...
unified_optimize(eligible, μ, Σ, params) -> weights:
  ...
  // Phase 2: optimizer 重みによる一括枝刈り
  //   → 上位 PRUNE_KEEP 個（例: 3 × max_holdings = 18）を残す
  //   → 残りは重みがほぼゼロなので情報損失は最小
  if n > PRUNE_KEEP:
    active = top_k_by_weight(w, PRUNE_KEEP)
  else:
    active = candidates  // 枝刈り不要

  // Phase 3: 縮小集合での厳密解
  //   → C(PRUNE_KEEP, max_holdings) サブセットの全列挙
  //   → 各サブセットで box_maximize_sharpe + box_risk_parity + blend
  //   → 最良 Sharpe のサブセットを選択
  //   → グローバル最適が保証される（枝刈り集合内で）
...
```
現行パイプライン:
  select_optimal_tokens()        ← ヒューリスティック（案 I で不要に）
  → maximize_sharpe_ratio()
  → apply_risk_parity()
  → alpha ブレンド
  → apply_constraints()          ← 後処理（案 I で不要に）
...
Phase 1 のブレンド重み上位 `PRUNE_KEEP` 個のトークンを残し、残りを除外する。
...
| パラメータ | 値 | 根拠 |
|---|---|---|
| `PRUNE_KEEP` | `3 × max_holdings` | Phase 3 の全列挙が現実的な上限。max_holdings=6 で PRUNE_KEEP=18 |
| 下限 | `max_holdings` | これ未満では Phase 3 の選択肢がない |
| 上限 | n（枝刈りなし） | n ≤ PRUNE_KEEP の場合は全トークンを Phase 3 に渡す |

`3 × max_holdings` の根拠:

- C(18, 6) = 18,564 サブセット → 各 O(6³) → 合計 ≈ 4.0M ops → ミリ秒以下
- C(24, 6) = 134,596 → まだ現実的だが、枝刈りバッファ 4× は過剰
- C(12, 6) = 924 → 2× は最小限だが、枝刈りで有用なトークンを失うリスクが上昇
- 3× は計算量と枝刈り信頼性のバランスが良好

#### 枝刈り信頼性分析

Phase 2 の枝刈りで「真の最適 max_holdings トークンが上位 PRUNE_KEEP に入らない」リスク:
...
- **重み上位 PRUNE_KEEP に入らないトークン**: Phase 1 の全体最適化で marginal
  contribution が低いと判定されたトークンであり、サブセットで最適化しても
  選ばれる確率は低い
...
exhaustive_optimize(active, μ_adj, Σ, params):
  ...
  for subset in combinations(active, max_holdings):
    ...
    // 閾値未満のトークンをゼロ化し、残りを再正規化
    for i in 0..len(w):
      if w[i] < min_position_size:
        w[i] = 0.0
    if sum(w) > 0:
      w = w / sum(w)
    else:
    ...
    // MIN_POSITION_SIZE 適用後に box 制約違反が起きうるため防御的 clamp
    apply_box_constraint_loop(&mut w, max_position)

    // Sharpe 評価（フルベクトルに埋め込んで計算）
  ...
  if best_weights is null:
    return equal_weight(active[0..max_holdings])
...
ゼロ化され、残り（例: 5 トークン）で再正規化される。
これにより `apply_constraints()` の MIN_POSITION_SIZE フィルタが統合される。

**ゼロ化後の再正規化で box 制約違反が起きるケース**:
再正規化で残存トークンの重みが max_position を超える可能性がある。
このため `apply_box_constraint_loop()`（Section 6 と同一ロジック）で
防御的 clamp を適用する。
...
- MIN_POSITION_SIZE フィルタによるゼロ化は max_holdings 個のサブセット内で処理され、
  結果として保有数 < max_holdings の解も評価される
- max_holdings 個のうち k 個がゼロ化された解は、同じ k 個を除外した
  max_holdings-k 個のサブセットでの解と類似（ただし同一ではない）
...
| n | Phase 1: O(n³) | Phase 2: O(n log n) | Phase 3: C(PRUNE_KEEP, k) × O(k³) | 合計 |
|---|---|---|---|---|
| 10 | O(1K) | → 10（枝刈り不要） | C(10, 6) × 216 = 45,360 | ≈ 46K ops |
| 30 | O(27K) | → 18 | C(18, 6) × 216 = 4.0M | ≈ 4.0M ops |
| 100 | O(1M) | → 18 | C(18, 6) × 216 = 4.0M | ≈ 5.0M ops |

- k = max_holdings = 6, PRUNE_KEEP = 3 × max_holdings = 18
- O(k³) = O(216) は box_maximize_sharpe + box_risk_parity の 1 サブセットあたりのコスト
  （アクティブセット法の反復を含む。k=6 では反復は数回で収束）
...
| 案 I（3 フェーズ） | ≈ 46K | ≈ 5M | Phase 3 の列挙がドミナント |

全ケースでミリ秒以下。Phase 3 の列挙が計算量のドミナントだが、
各サブセットの最適化は k=6 の極小問題であり、実時間への影響は軽微。

#### PRUNE_KEEP を変えた場合の Phase 3 計算量

| PRUNE_KEEP | C(PRUNE_KEEP, 6) | 合計 ops | 評価 |
|---|---|---|---|
| 12（2×） | 924 | 200K | 最小。枝刈り信頼性にやや不安 |
| 18（3×） | 18,564 | 4.0M | 推奨。計算量と信頼性のバランス |
| 24（4×） | 134,596 | 29M | 許容範囲だが過剰 |
| 30（5×） | 593,775 | 128M | やや重い。n=100 でも不要 |
...
|---|---|---|---|---|
| 実装コスト | 低（~50 行） | 中（~100 行） | 高（数百行+外部依存） | 中〜高（~450 行） |
| 新設関数 | 0 | 1 | — | 7-8 |
| 既存関数改修 | 2 | 1 | — | 1 |
...
| 最適化実行回数 | 1 | 2 | 1 | 1 + C(PRUNE_KEEP, k) |
...
- **select_optimal_tokens() 廃止**: ヒューリスティックスコアリングが不要になり、
  optimizer 自身の判断でトークンを選定。共分散構造の完全活用
- **apply_constraints() 廃止**: 後処理の歪みが構造的に解消。
  全制約が最適化ループ内で処理される
- **グローバル最適保証**: Phase 3 の全列挙により、枝刈り集合内での
  最良サブセットが保証される（案 G のセーフガードよりも強い保証）
- **スケーラビリティ**: n=100 でもミリ秒以下の計算量

#### 案 I の弱点

- **実装コスト**: ~450 行の新規コード（案 D の ~50 行と比較して約 9 倍）
- **Ledoit-Wolf 前提**: n > 30 では Phase 1 の信頼性が Σ の品質に依存
- **枝刈りリスク**: Phase 2 の枝刈りで有用なトークンが除外される可能性
  （Section 15.4 の信頼性分析参照）
...
| `unified_optimize()` | ~120 | 3 フェーズ統合ループ |
| `exhaustive_optimize()` | ~60 | 全サブセット列挙 + 最良選択 |
...
| `test_unified_small_n` | n ≤ max_holdings で枝刈りスキップ |
...
| `test_pruning_preserves_top_tokens` | 枝刈りが高品質トークンを保持 |
...
- `apply_constraints()` の後処理除去
- Phase 3 の全列挙によるグローバル最適保証（C(10,6)=210 サブセット）

ただし:
- 案 D が主要な最適性ロス（MAX_HOLDINGS 脱落）を既に解消
- ~450 行の実装コストに対し、n=10 での追加改善幅は限定的
