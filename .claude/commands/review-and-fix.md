...
9. **チーム破棄**: 全 reviewer + coordinator に `shutdown_request` を SendMessage → 完了通知後 `TeamDelete(team_name="code-review-<run-id>")`
...
5. 全合意後、**まず `findings.json` を Write で出力し、書き込み成功を確認してから** DONE メッセージを team-lead に SendMessage する（順序を逆にしないこと。書き込み前に DONE を送ると team-lead が存在しないファイルを参照する）
...
親はファイルを Read しないため、CP1 のユーザ判断材料はこの summary のみ。1 finding = 1 行、フィールド区切りは ASCII パイプ `|`（前後に空白を入れない）。
...
9. **チーム破棄**: plan-coordinator + 起動済み specialist に `shutdown_request` を SendMessage → 完了通知後 `TeamDelete(team_name="plan-<run-id>")`
...
6. **まず `groups.json` を Write で出力し、書き込み成功を確認してから** DONE メッセージを team-lead に SendMessage する（順序厳守）
...
親はファイルを Read しないため、CP2 のユーザ判断材料はこの summary のみ。1 グループ = 1 行、フィールド区切りは ASCII パイプ `|`（前後に空白を入れない）。
...
5. 全グループ完了後、**`results.json` を Write で出力（commit 一覧、skipped、deviation 履歴を含む）してから** ALL_DONE メッセージを team-lead に SendMessage する
...
親はファイルを Read しないため、最終サマリ表示はこの ALL_DONE のみ。1 グループ = 1 行、フィールド区切りは ASCII パイプ `|`（前後に空白を入れない）。
