...
fn test_find_fallback_path_returns_nearest_newer() {
    ...
    let rates = vec![
        TokenRate {
            ...
            quote: quote.clone(),
            exchange_rate: make_rate(1000),
            timestamp: now - chrono::TimeDelta::hours(4),
            rate_calc_near: 10,
            swap_path: None,
        },
        TokenRate {
            ...
            quote: quote.clone(),
            exchange_rate: make_rate(1000),
            timestamp: now - chrono::TimeDelta::hours(3),
            rate_calc_near: 10,
            swap_path: None,
        },
        TokenRate {
...
fn test_precompute_fallback_indices_performance() {
    ...
    let rates: Vec<TokenRate> = (0..n)
        .map(|i| TokenRate {
            ...
            timestamp: now - chrono::TimeDelta::minutes(n as i64 - i as i64),
            rate_calc_near: 10,
            swap_path: if i % 10 == 9 {
    ...
    let generate_rates = |n: usize| -> Vec<TokenRate> {
        (0..n)
            .map(|i| TokenRate {
                ...
                quote: quote.clone(),
                exchange_rate: make_rate(1000 + i as i64),
                timestamp: now - chrono::TimeDelta::seconds(n as i64 - i as i64),
                rate_calc_near: 10,
                swap_path: if i % 10 == 9 {
                    Some(make_path(i as u32))
                } else {
...
fn test_to_spot_rates_preserves_timestamp_order() {
    ...
    let ts0 = now - chrono::TimeDelta::hours(3);
    let ts1 = now - chrono::TimeDelta::hours(2);
    let ts2 = now - chrono::TimeDelta::hours(1);
