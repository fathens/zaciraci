use super::helpers::{RATE_24, buy_op, sell_op, token_account};
use super::*;
...
fn test_exact_match_single_pair() {
    ...
    assert_eq!(
        result.direct_swaps[0].sell_token,
        token_account("token_a.near")
    );
    assert_eq!(
        result.direct_swaps[0].buy_token,
        token_account("token_b.near")
    );
    assert_eq!(
...
fn test_sell_greater_than_buy() {
    ...
    assert_eq!(
        result.remaining_sells[0].token,
        token_account("token_a.near")
    );
    assert_eq!(
...
fn test_multiple_sells_multiple_buys_exact() {
    ...
    assert_eq!(
        result.direct_swaps[1].sell_token,
        token_account("token_a.near")
    );
    assert_eq!(
        result.direct_swaps[1].buy_token,
        token_account("token_d.near")
    );
    assert_eq!(
    ...
    assert_eq!(
        result.direct_swaps[2].sell_token,
        token_account("token_c.near")
    );
    assert_eq!(
        result.direct_swaps[2].buy_token,
        token_account("token_d.near")
    );
    assert_eq!(
...
fn test_multiple_sells_vs_single_buy_remainder() {
    ...
    assert_eq!(
        result.remaining_sells[1].token,
        token_account("token_b.near")
    );
    assert_eq!(
    ...
    assert_eq!(
        result.remaining_sells[2].token,
        token_account("token_c.near")
    );
    assert_eq!(
...
#[test]
fn test_invariant_fractional_near_values() {
    // 端数を含む NearValue でのマッチング分割時に BigDecimal 精度が保たれることを検証
    // sells=[A(33.333), B(16.667)], buys=[C(25.5), D(24.5)]
    let sells = vec![
        SellOperation {
    ...
    let buys = vec![
        ...
        BuyOperation {
            ...
            near_value: NearValue::from_near(BigDecimal::from_str("24.5").unwrap()),
        },
    ];

    let total_sell: NearValue = NearValue::from_near(BigDecimal::from_str("50.000").unwrap());
    let total_buy: NearValue = NearValue::from_near(BigDecimal::from_str("50.0").unwrap());
    ...
    assert_eq!(
        ...
        total_sell,
        "sell invariant broken with fractional values"
    );
