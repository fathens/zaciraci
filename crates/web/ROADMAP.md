...
## Phase 1: Config API (完了)

- HealthService: DB 接続チェック付きヘルスチェック
- ConfigService: 設定の CRUD (GetAll, GetOne, Upsert, Delete)
  - `persistence::config_store` の既存関数を直接利用

## Phase 2: ポートフォリオ・取引閲覧 (完了)

- PortfolioService (`portfolio.proto`)
  - GetEvaluationPeriods: 評価期間一覧
  - GetEvaluationPeriod: 評価期間詳細
  - GetSelectedTokens: 評価期間の選定トークン一覧
  - GetTrades: 取引履歴 (ページネーション、評価期間フィルタ)
  - GetTradesByBatch: バッチ単位の取引詳細
  - GetLatestBatch: 最新バッチ
  - GetLatestRates: 全トークン最新レート
  - GetRateHistory: 複数トークンのレート履歴 (start/end 指定)

利用する既存関数:
- `persistence::evaluation_period::EvaluationPeriod` (get_all_async, get_by_period_id_async)
- `persistence::trade_transaction::TradeTransaction` (find_paginated_async, find_by_batch_id_async, get_latest_batch_id_async)
- `persistence::token_rate` (get_all_latest, get_rates_for_multiple_tokens_simple)
