...
fn convert_to_f64(decimal: &BigDecimal) -> Result<f64> {
    decimal
        .to_f64()
        .ok_or_else(|| anyhow!(TimeSeriesPredictionError::ConversionError(
            format!("Failed to convert BigDecimal to f64: {}", decimal)
        )))
}
...
fn convert_from_f64(value: f64) -> Result<BigDecimal> {
    BigDecimal::try_from(value)
        .map_err(|e| anyhow!(TimeSeriesPredictionError::ConversionError(
            format!("Failed to convert f64 to BigDecimal: {}: {}", value, e)
        )))
}
...
/// Vecをndarray形式に変換する
fn vec_to_ndarray(features: Vec<Vec<f64>>, targets: Vec<f64>) -> Result<(Array2<f64>, Array1<f64>)> {
    ...
    let x = Array2::from_shape_vec((n_samples, n_features), x_data)
        .map_err(|e| anyhow!(TimeSeriesPredictionError::ConversionError(
            format!("Failed to convert features to ndarray: {}", e)
        )))?;
...
/// 線形回帰モデルを訓練する
fn train_linear_model(features: Vec<Vec<f64>>, targets: Vec<f64>) -> Result<linfa_linear::FittedLinearRegression<f64>> {
    ...
    let model = LinearRegression::default()
        .fit(&dataset)
        .map_err(|e| anyhow!(TimeSeriesPredictionError::PredictionError(
            format!("Failed to train linear model: {}", e)
        )))?;
...
pub fn predict_future_rate(
    ...
    if target_time <= latest_time {
        return Err(anyhow!(TimeSeriesPredictionError::InvalidTimeRange(
            format!("Target time must be in the future: latest={:?}, target={:?}", 
    ...
    for i in 0..steps_ahead {
        // 現在の特徴量からの予測
        let x_pred = Array2::from_shape_vec((1, LAG_COUNT), current_features.clone())
            .map_err(|e| anyhow!(TimeSeriesPredictionError::ConversionError(
                format!("Failed to convert prediction features to ndarray: {}", e)
            )))?;
