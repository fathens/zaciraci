...
fn test_extreme_loss_scenarios() {
    // 極端損失シナリオの分析（VaR代替実装）
    let mut historical_returns = vec![
        0.05, -0.02, 0.08, -0.15, 0.03, 0.12, -0.08, 0.01, -0.05, 0.09,
        -0.12, 0.04, -0.03, 0.07, -0.18, 0.06, -0.01, 0.02, -0.09, 0.11
    ];
...
fn test_liquidity_impact_on_weights() {
    ...
    let liquidity_based_weights: Vec<f64> = tokens
        .iter()
        .map(|token| token.liquidity_score)
        .collect();
...
fn test_portfolio_optimization_robustness() {
    ...
    let weight_changes: f64 = weights_stable.iter()
...
fn test_simple_concentration_analysis() {
    ...
    // 高集中ポートフォリオ
    let concentrated_weights = vec![0.7, 0.2, 0.1];
    // 分散されたポートフォリオ
    let diversified_weights = vec![0.33, 0.33, 0.34];
...
fn test_performance_metrics_with_existing_functions() {
    ...
    let portfolio_returns = vec![
        0.05, 0.03, -0.02, 0.08, -0.01, 0.06, 0.04, -0.03, 0.07, -0.05,
        0.02, 0.09, -0.04, 0.05, -0.02, 0.03, 0.08, -0.01, 0.04, 0.06
    ];
