...
impl DataNormalizer {
    ...
    fn apply_moving_average(&self, data: &[ValueAtTime]) -> Result<Vec<ValueAtTime>, String> {
        ...
        // 初期ウィンドウを構築
        for i in 0..self.moving_average_window.min(data.len()) {
    ...
    fn interpolate_value(&self, data: &[ValueAtTime], index: usize) -> Result<f64, String> {
        ...
        if index >= data.len() - 1 {
            // 最後の要素の場合、前の有効な値を使用
            return Ok(data.get(data.len() - 2).map(|v| v.value).unwrap_or(data[data.len() - 1].value));
        }
    ...
    /// データの品質指標を計算
    pub fn calculate_data_quality_metrics(&self, original: &[ValueAtTime], normalized: &[ValueAtTime]) -> DataQualityMetrics {
        let original_variance = self.calculate_variance(&original.iter().map(|v| v.value).collect::<Vec<_>>());
        let normalized_variance = self.calculate_variance(&normalized.iter().map(|v| v.value).collect::<Vec<_>>());
