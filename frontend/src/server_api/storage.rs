...
impl StorageApi {
    ...
    pub async fn deposit(&self, amount: &str) -> String {
        self.underlying.get_text(&format!("storage/deposit/{amount}")).await
    }

    pub async fn unregister_token(&self, token_account: &str) -> String {
        self.underlying.get_text(&format!("storage/unregister/{token_account}"))
            .await
    }
    ...
    pub async fn amounts_wrap(&self, amount: &str) -> String {
        self.underlying.get_text(&format!("storage/amounts/wrap/{amount}"))
            .await
    }

    pub async fn amounts_unwrap(&self, amount: &str) -> String {
        self.underlying.get_text(&format!("storage/amounts/unwrap/{amount}"))
            .await
    }

    pub async fn amounts_deposit(&self, token_account: &str, amount: &str) -> String {
        self.underlying.get_text(&format!("storage/amounts/deposit/{token_account}/{amount}"))
            .await
    }

    pub async fn amounts_withdraw(&self, token_account: &str, amount: &str) -> String {
        self.underlying.get_text(&format!(
