...
impl PoolsApi {
    ...
    pub async fn estimate_return(&self, pool_id: &str, amount: &str) -> String {
        self.underlying.get_text(&format!("pools/estimate_return/{pool_id}/{amount}"))
            .await
    }

    pub async fn get_return(&self, pool_id: &str, amount: &str) -> String {
        self.underlying.get_text(&format!("pools/get_return/{pool_id}/{amount}"))
            .await
    }
    ...
    pub async fn list_returns(&self, token_account: &str, amount: &str) -> String {
        self.underlying.get_text(&format!("pools/list_returns/{token_account}/{amount}"))
            .await
    }

    pub async fn pick_goals(&self, token_account: &str, amount: &str) -> String {
        self.underlying.get_text(&format!("pools/pick_goals/{token_account}/{amount}"))
            .await
    }
