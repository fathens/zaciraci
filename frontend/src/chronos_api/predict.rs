...
mod tests {
    ...
    fn test_parse_datetime_samples() {
        ...
        // タイムスタンプのパースをテスト
        let parsed_timestamps = parse_datetime_vec(&sample_timestamps).expect("タイムスタンプのパースに失敗");
        assert_eq!(parsed_timestamps.len(), 3);
        
        // 予測時点のパースをテスト
        let parsed_forecast_until = parse_datetime(sample_forecast_until).expect("予測時点のパースに失敗");
        ...
        assert_eq!(
            parsed_forecast_until.format("%Y-%m-%d %H:%M:%S").to_string(),
            "2023-01-04 02:00:00"
        );
    ...
    fn test_request_json_compatibility() {
        ...
        let request = ZeroShotPredictionRequest {
            ...
            forecast_until: parse_datetime("2023-01-04T02:00:00").expect("forecast_untilのパースに失敗"),
            model_name: Some("chronos_default".to_string()),
            model_params: {
                let mut params = HashMap::new();
                params.insert("seasonality_mode".to_string(), serde_json::json!("multiplicative"));
                Some(params)
            },
        };
        ...
        let deserialized: ZeroShotPredictionRequest = serde_json::from_str(&serialized).expect("デシリアライズに失敗");
