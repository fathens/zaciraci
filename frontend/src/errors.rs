...
impl Display for PredictionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let message = match self {
            ...
            PredictionError::VolatilityTokensNotFound => "ボラティリティトークンが見つかりませんでした",
            PredictionError::InsufficientData => "予測用のデータが不足しています", 
            PredictionError::InsufficientDataAfterSplit => "データ分割後のデータが不足しています",
            
            // パース関連  
            PredictionError::QuoteTokenParseError(detail) => return write!(f, "Quote tokenのパースエラー: {}", detail),
            PredictionError::BaseTokenParseError(detail) => return write!(f, "Base tokenのパースエラー: {}", detail),
            PredictionError::StartDateParseError(detail) => return write!(f, "開始日時のパースエラー: {}", detail),
            PredictionError::EndDateParseError(detail) => return write!(f, "終了日時のパースエラー: {}", detail),
            
            // API関連
            PredictionError::PredictionApiError(detail) => return write!(f, "予測実行エラー: {}", detail),
            PredictionError::VolatilityTokensApiError(detail) => return write!(f, "ボラティリティトークン取得エラー: {}", detail),
            PredictionError::RequestError(detail) => return write!(f, "リクエストエラー: {}", detail),
            
            // システム関連
            PredictionError::QuoteTokenSetupError => "quote_tokenの設定に失敗しました",
            PredictionError::ChartGenerationError(detail) => return write!(f, "チャート生成エラー: {}", detail),
            PredictionError::SvgGenerationError(detail) => return write!(f, "SVG生成エラー: {}", detail),
            ...
            // テスト関連（予約）
            PredictionError::EmptyPredictionDataError(detail) => return write!(f, "空の予測データでエラー: {}", detail),
            PredictionError::EmptyTestDataError(detail) => return write!(f, "空のテストデータでエラー: {}", detail),
        };
        write!(f, "{}", message)
    }
}
